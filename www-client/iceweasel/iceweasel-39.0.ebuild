...
# Patch version
PATCH="firefox-38.0-patches-0.3"
# Upstream ftp release URI that's used by mozlinguas.eclass
# We don't use the http mirror because it deletes old tarballs.
MOZ_FTP_URI="ftp://ftp.mozilla.org/pub/firefox/releases"
MOZ_HTTP_URI="http://ftp.mozilla.org/pub/firefox/releases"
...
DESCRIPTION="Iceweasel a Fork of Firefox Web Browser"
...
IUSE="+iceweasel egl hardened +minimal neon pgo selinux +gmp-autoupdate test"
...
pkg_setup() {
	...
	if ! use iceweasel; then
		...
		elog "You can disable it by emerging ${PN} _with_ the iceweasel USE-flag"
	fi
...
src_prepare() {
	...
	#Apply Iceweasel branding
	if use iceweasel ; then 
		epatch "${FILESDIR}/Determine-which-phishing-shavar-to-use-depending-on-.patch"
		epatch "${FILESDIR}/Modify-search-plugins-depending-on-MOZ_APP_NAME.patch"
		epatch "${FILESDIR}/Use-MOZ_APP_DISPLAYNAME-to-fill-appstrings.propertie.patch"
		epatch "${FILESDIR}/Use-firefox-instead-of-MOZ_APP_NAME-for-profile-rese.patch"
	fi
...
src_install() {
	...
	# newicon "${icon_path}/icon48.png" "${icon}.png"
	newmenu "${FILESDIR}/branding/${PN}.desktop" "${PN}.desktop"
