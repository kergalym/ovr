...
SLOT="2"
IUSE="debug device-mapper doc efiemu +fonts mount +multislot nls static sdl test +themes truetype libzfs +branding"
...
src_install() {
	...
	insinto /etc/default
	newins "${FILESDIR}"/grub.brand-2 grub
	doins "${FILESDIR}"/background.png
	fi
}
