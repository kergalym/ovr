...
start() {
   ...
   if [ ${MACHINE_TYPE} == 'x86_64' ]; then
	  ...
	  cp $BUILDROOT/etc/kernels/*x86* $BUILDDATA/scripts/
          ...
          rm $BUILDROOT/etc/portage/make.conf
          chroot ${BUILDROOT} /bin/bash -c 'ln -s /etc/portage/make32xfce.conf /etc/portage/make.conf'
...
if  [ -e $BUILDDATA/.work.session ] ; [ -a $LOOP_IMAGE ] ; then
    ...
    sleep 1
    wget -c -O $BUILDDATA/stage3-i686.tar.bz2 $STAGE3
    ...
    sleep 1
    tar xpf $BUILDDATA/stage3*i686*.tar.bz2 -C $BUILDROOT 2> $BUILDROOT/build.log
    cp -L /etc/resolv.conf $BUILDROOT/etc/
